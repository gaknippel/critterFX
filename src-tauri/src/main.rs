...
fn scan_ae_installations() -> Vec<AEInstallation> {
    //main fn for scanning your AE
    let mut installations = Vec::new();

    //ae from 2025 to 2020
    let versions = vec!["2025", "2024", "2023", "2022", "2021", "2020"];
...
#[tauri::command]
fn get_path_config(app: tauri::AppHandle) -> Result<PathConfig, String> {
    //can succeed and return path config, or fail and return message
    let app_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;

    let config_path = app_dir.join("path_config.json");

    if config_path.exists() {
        let content = fs::read_to_string(&config_path).map_err(|e| e.to_string())?; //error mapping

        let config: PathConfig = serde_json::from_str(&content).map_err(|e| e.to_string())?; //error mapping

        Ok(config)
    } else {
        //will execute first time launch, since no custom stuff
        Ok(PathConfig {
...
fn install_preset(
    ...
    println!("success!");
    Ok(format!(
        "successfully installed {} to after effects",
        file_name
    ))
}
